package RapidApp::Role::ModuleController;
use Moose::Role;
...
## --------------------------------------------------------------
##
##      RapidApp::Role::ModuleController
##
## Merger of legacy roles which have been DEPRECATED:
##
##  * RapidApp::Role::Module
##  * RapidApp::Role::Controller
##
## The original rationales behind why these were separate
## no longer apply, and have been combined here in order to 
## facilitate future refactoring plans... (GitHub Issue #74)
##
##
## --------------------------------------------------------------
...
has 'parent_module_ref'  => ( is => 'ro',  isa => 'Maybe[RapidApp::Role::ModuleController]', weak_ref => 1, required => 1);
...
sub create_module {
  ...
  die "$class_name is not a valid RapidApp Module" unless ($Object->does('RapidApp::Role::ModuleController'));
  ...
  foreach my $Handler (@_) {
    ...
    if($self->print_rapidapp_handlers_call_debug) {
      ...
      if (defined $Handler->scope) {
        ...
        if ($Handler->scope->does('RapidApp::Role::ModuleController')) {
...
RapidApp::Role::ModuleController - Role for RapidApp Modules
