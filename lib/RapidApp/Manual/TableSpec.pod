...
single relationship, or belongs_to). Current supported values are C<grid>, C<combo> and C<custom>.
...
C<combo> sets up a simple dropdown box, with the items listed showing the value of the 
C<display_column> for each row. C<combo> is a nice alternative to C<grid> when there is a limited 
number of rows to select from.
...
Boolean. Hides the column from being shown/selected as a column in grid views. Does not effect other
