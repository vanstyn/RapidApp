...
sub Params {
	...
	unless (defined $self->{Params}) {
		...
		#$p->{loadMask}		= \1				unless (defined $p->{loadMask});
...
sub column_model {
	...
	#push @{$a}, 'thisG.expander' if ($self->{Params}->{expander_template});
	...
	foreach my $field ($self->field_list) {
		...
		# --- GridFilter:
		#$h->{filterable} = \1;
		#$h->{data_type} = 'string' unless (defined $h->{data_type});
		#$h->{filter} = { type => $h->{data_type} };
		#$h->{filter} = { type => 'boolean' } if ($h->{checkbox});
		#$h->{filter} = { type => 'list', options => $h->{enum_list} } if (defined $h->{enum_list} and ref($h->{enum_list}) eq 'ARRAY');
		# ---
